...
########################
led_pin = 7 # LED 
btn_pin = 40 # pin for the start button
...
high_res_preview_w = 1280 # a lower resolution width for the preview. The height is calculated
	...
	if config.post_online: # turn off posting pics online in config.py
		...
		while connected: 
			try:
				...
				msg = "#mfb16 Raspberry Jam Berlin Photobooth powered by #raspberrypi"
				if len(msg) <= 140:
					api.update_with_media(file_to_upload, status=msg)
				else:
					print "tweet not sent. Too long. 140 chars Max."
				break
			except ValueError:
